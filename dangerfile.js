...
if (danger.github && danger.github.pr) {
  ...
  for (const { commit } of ghCommits) {
    ...
    if (!firstLineRegex.test(firstLine)) {
      fail(
        `"[${firstLine}](${url})" is not using a valid commit message format. For commit guidelines, see: [CONTRIBUTING](https://github.com/repocheck/repocheck/blob/master/CONTRIBUTING.md#creating-commits).`,
      );
    }
  ...
  // Warn if changes to help files are created in repocheck/repocheck repo instead of repocheck/user-docs
  ...
  if (modifiedHelp || createdHelp) {
    warn(
      'Please make changes to `repocheck help` text in [Gitbook](https://docs.repocheck.io/features/repocheck-cli/commands). Changes will be automatically synchronised to Repocheck CLI as a [scheduled PR](https://github.com/repocheck/repocheck/actions/workflows/sync-cli-help-to-user-docs.yml).\nFor more information, see: [`help/README.md`](https://github.com/repocheck/repocheck/tree/master/help/README.md).',
    );
  }
}
